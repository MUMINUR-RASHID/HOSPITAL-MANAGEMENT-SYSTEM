	...
	public Updatecheck() throws SQLException {
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) { 
				...
				String s2 = txt_ID.getText(); //room_number;    
                                ...
                                c.s.executeUpdate("update ADDMISSION set room = '"+s2+"', name = '"+s3+"', check_in = '"+s4+"', deposite = '"+s5+"' where number = '"+s1+"'");
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String s1 = c1.getSelectedItem();
				Conn c = new Conn();
                                ResultSet rs1 = c.s.executeQuery("select * from ADDMISSION where number = "+s1);
                                
                                while(rs1.next()){
                                    txt_ID.setText(rs1.getString("room"));    
                                    txt_Status.setText(rs1.getString("name"));    
                                    txt_Date.setText(rs1.getString("check_in"));    
                            ...
                            try{
                                String total = "";
                                Conn c  = new Conn();
                                ResultSet rs2 = c.s.executeQuery("select * from CABINE where room = "+txt_ID.getText());
                                while(rs2.next()){
